  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'a':
          if(verbose_flag) printf("Appending bank file %s to stored transactions\n", optarg);
          append = 1;
          appendFile = optarg;
          appendRoutine(appendFile);
          break;

        case 'u':
          if(verbose_flag) puts ("Updating transaction list\n");
          update = 1;
          break;
          
        case 'm':
             if(verbose_flag) printf("Calculating monthly sum for tags: %s\n", optarg);
             calc_month_sum = 1;
		...
		case 'x':
			if(verbose_flag) printf("Show stored tags\n");
			printTagsRoutine(verbose_flag);
			break;
		
		case 's':
			if(verbose_flag) printf("Show keywords in tag:%s\n", optarg);
			printKeyWordsRoutine(optarg, verbose_flag);
			break;
		
		case 't':
			if(verbose_flag) printf("Adding tag:%s\n", optarg);
			addTagRoutine(optarg, verbose_flag);
			break;
		
		case 'k':
			if(verbose_flag) printf("Adding keyword:%s\n", optarg);
			addKeywordRoutine(optarg, verbose_flag);
			break;
		
		case 'd':
			if(verbose_flag) printf("Show transaction interval.\n");
