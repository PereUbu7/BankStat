...
int removeTwinTransElements(struct transLList *trans, int verbose)
{
    ...
    storedTrans = findFirstElement(trans, verbose);
...
struct transLList *gnomeSortTransLList(struct transLList *listStart, int verbose)
{
       ...
       while( upperBound != NULL )
       {
              ...
              while( currentElement->prev != NULL &&  !cmpTransDates(currentElement->prev, currentElement, verbose) )
              {
   ...
   return(findFirstElement(currentElement, verbose));    
